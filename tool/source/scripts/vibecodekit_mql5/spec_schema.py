...
from vibecodekit_mql5.spec_blocks_extra import (
...
from vibecodekit_mql5.spec_extensions import (
    PropFirmConfig,
    StealthConfig,
    TimeExitConfig,
    validate_prop_firm,
...
from vibecodekit_mql5.spec_extensions import (
    check_unknown_keys as _check_unknown_keys,  # noqa: F401 -- compatibility re-export
)
...
def _validate_compatibility(errors: list[str], raw: Any) -> None:
    if raw is None:
        return
    if not isinstance(raw, dict):
        errors.append(f"spec.compatibility must be a mapping, got {type(raw).__name__}")
        return
    unknown = set(raw) - {"mode", "release_eligible"}
    if unknown:
        errors.append(f"spec.compatibility has unknown keys: {sorted(unknown)}")
    if raw.get("mode") != "legacy_scaffold":
        errors.append("spec.compatibility.mode must be 'legacy_scaffold'")
    if raw.get("release_eligible") is not False:
        errors.append("spec.compatibility.release_eligible must be false")
...
def validate(
    ...
    *,
    ...
    _validate_compatibility(errors, spec.get("compatibility"))
    ...
    unknown_top = set(spec.keys()) - {
        *REQUIRED_TOP_FIELDS, "mode", "risk", "signals", "filters", "hooks",
        "prop_firm", "time_exit", "stealth", "compatibility",
...
def render_signals_doc(spec: EaSpec) -> str:
    ...
    lines.append(
        ...
        f"the entry/exit logic in `{spec.name}.mq5`._"
    )
    return "\n".join(lines) + "\n"
