...
def find_evidence(
    ...
    *,
    ...
    for rel, text in files.items():
        ...
        scan_lines = strip_comments_and_strings(text).splitlines() if code_only else raw_lines
        for pat in patterns:
            ...
            for idx, line in enumerate(scan_lines, start=1):
                if code_only and _INCLUDE_LINE.match(line):
                    continue
                if rx.search(line):
                    snippet = (raw_lines[idx - 1] if idx - 1 < len(raw_lines) else line).strip()[
                        :220
                    ]
...
def make_claim_ledger(project: str | Path) -> dict[str, Any]:
    ...
    entry_files = {k: v for k, v in files.items() if k.lower().endswith(".mq5")} or files
    weak_claims = {"news_filter", "spread_filter", "ml_filter"}
    claim_defs = [
        (
            "hedge_grid_initial_entries",
            "EA can open initial Buy/Sell hedge positions",
            [r"OpenInitialHedge", r"BuyFast\s*\(", r"SellFast\s*\("],
        ),
        (
            "smc_breaker_detection",
            "EA detects buy/sell breaker from swing high/low",
            [
                r"SMCDetector",
                r"BUY_BREAKER",
                r"SELL_BREAKER",
        ...
        (
            ...
            "EA uses async trade execution",
            [r"CAsyncTradeExecutor", r"SetAsyncMode\s*\(\s*true\s*\)", r"OnTradeTransaction"],
        ),
        (
            "async_losing_side_close",
            "EA closes losing side asynchronously on breaker",
            [r"ReduceLosingSideOnBreaker", r"CloseSideFast"],
        ),
        (
            "async_basket_close",
            "EA closes basket asynchronously",
            [r"CloseBasketFast", r"VirtualTPAndBasketTrail"],
        ),
        (
            "grid_level_limit",
            "EA limits grid levels",
            [r"InpMaxLevels", r"LevelAllowed", r"m_max_levels"],
        ),
        (
            "drawdown_freeze",
            "EA freezes new exposure at drawdown threshold",
            [r"InpFreezeDDPercent", r"Freeze\s*\(", r"ShouldFreeze"],
        ),
        (
            "drawdown_hard_stop",
            "EA stops opening exposure at max drawdown",
            [r"InpMaxDDPercent", r"Stop\s*\(", r"MustStop"],
        ),
        (
            "persistent_state",
            "EA persists state",
            [r"CPersistentStateStore", r"GlobalVariable", r"PersistentState\.Save"],
        ),
        (
            "structured_logging",
            "EA writes structured logs",
            [r"CStructuredLogger", r"StructuredLog", r"\{\\\"level\\\""],
        ),
        (
            "account_seed_divergence",
            "EA uses account seed divergence",
            [r"AccountSeed", r"ACCOUNT_LOGIN", r"InpUseAccountSeedDivergence"],
        ),
        (
            "virtual_tp",
            "EA uses VirtualTP basket close threshold",
            [r"InpVirtualTPPoints", r"m_virtual_tp", r"SYMBOL_TRADE_TICK_VALUE"],
        ),
        (
            "basket_trailing_full",
            "EA implements full basket trailing",
            [r"trailing_stop", r"trail_lock", r"BasketTrail", r"m_trail_start"],
        ),
        (
            "news_filter",
            "EA has a news filter",
            [
                r"CalendarValueHistory",
                r"CalendarValue\w*\s*\(",
                r"\bCNewsFilter\b",
                r"\bIsNewsTime\b",
                r"\bInpUseNews\w*\b",
    ...
    for cid, desc, pats in claim_defs:
        ...
        if cid in {"async_trade_execution"}:
            supported = bool(find_evidence(files, [r"SetAsyncMode\s*\(\s*true\s*\)"])) and bool(
                find_evidence(files, [r"OnTradeTransaction"])
            )
        if cid in {"async_losing_side_close"}:
            supported = bool(find_evidence(files, [r"ReduceLosingSideOnBreaker"])) and bool(
                find_evidence(files, [r"CloseSideFast"])
            )
        if cid in {"async_basket_close"}:
            supported = bool(find_evidence(files, [r"CloseBasketFast"])) and bool(
                find_evidence(files, [r"VirtualTPAndBasketTrail"])
            )
    ...
    return {
        "schema_version": "1.0",
        "artifact_type": "ea_doc_claim_ledger",
        "project": str(project),
        "claims": claims,
    }
...
_RELEASE_CLAIM = re.compile(
    r"\b(?:production[- ]?ready|live[- ]?ready|release[- ]?eligible)\b", re.I
)
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        prog="ea-doc-claims",
        description="Audit EA documentation claims against source and release evidence.",
    )
