...
_FUNC = re.compile(
    r"\b(OnTick|OnTimer|OnDeinit|OnChartEvent)\s*\([^)]*\)\s*\{(?P<body>.*?)\n?\}", re.S
)


def _has_ui_context(src: str) -> bool:
    return bool(
        re.search(
            r"\b(?:OnChartEvent|ObjectCreate|ObjectSetString|ObjectSetInteger|"
            r"ChartRedraw|RenderPanel|DrawPanel|UpdatePanel)\s*\(|"
            r"\b(?:OBJ_[A-Z0-9_]+|VCKP_|panel_)",
            src,
            re.I,
        )
    )
...
def detect_ux04(path, raw, src):
    if not re.search(r"\bObjectCreate\s*\(", src):
        return []
    ...
    if has_prefix and cleanup:
        return []
    m = re.search(r"\bObjectCreate\s*\(", src)
    ...
    return [
        Finding(
            ...
            "ERROR",
            "UX-04",
            "Panel objects need a stable prefix and deterministic OnDeinit cleanup",
        )
    ]
...
def detect_ux05(path, raw, src):
    out = []
    blocked = re.compile(
        r"\b(?:Alert|MessageBox|Sleep|WebRequest|FileOpen|ChartRedraw|iCustom|iMA|iRSI|iATR)\s*\("
    )
    for name, body, start in _bodies(src):
        if name not in {"OnTick", "OnChartEvent"}:
            continue
        for m in blocked.finditer(body):
            ...
            out.append(
                Finding(
                    ...
                    "ERROR",
                    "UX-05",
                    f"{m.group(0).split('(')[0]} in {name} may block the EA event queue",
                )
            )
    return out
...
def detect_ux06(path, raw, src):
    if not re.search(r"\bObjectCreate\s*\(", src):
        return []
    if re.search(
        r"(?:CORNER_|anchor|dpi|CHART_WIDTH_IN_PIXELS|CHART_HEIGHT_IN_PIXELS|OBJPROP_CORNER)",
        src,
        re.I,
    ):
        return []
    ...
    return [
        Finding(
            ...
            "WARN",
            "UX-06",
            "Panel layout lacks an explicit anchor/resize/DPI strategy",
        )
    ]


def detect_ux07(path, raw, src):
    if not re.search(r"\bObjectSetInteger\s*\([^;]*(?:OBJPROP_COLOR|OBJPROP_TEXT_COLOR)", src):
        return []
    if re.search(r"OBJPROP_BGCOLOR|OBJPROP_BACK|OBJ_RECTANGLE_LABEL|panel_bg", src, re.I):
        return []
    m = re.search(r"OBJPROP_COLOR", src)
    line, col = _line_col(src, m.start())
    return [
        Finding(path, line, col, "WARN", "UX-07", "Panel text color has no owned contrast surface")
    ]


def detect_ux08(path, raw, src):
    if "ChartRedraw" not in src:
        return []
    if re.search(r"dirty|redraw.*(?:timer|interval|elapsed)|last_redraw", src, re.I):
        return []
    m = re.search(r"ChartRedraw", src)
    line, col = _line_col(src, m.start())
    return [
        Finding(
            path,
            ...
            "WARN",
            "UX-08",
            "ChartRedraw is not visibly guarded by dirty state and cadence",
        )
    ]


def detect_ux09(path, raw, src):
    if not _has_ui_context(src):
        return []
    if not re.search(
        r"(?:close.?all|emergency|disable.?strategy|modify.?order|destructive)", src, re.I
    ):
        return []
    if re.search(r"(?:confirm|nonce|armed|two.?step|risk.?guard)", src, re.I):
        return []
    m = re.search(
        r"(?:close.?all|emergency|disable.?strategy|modify.?order|destructive)", src, re.I
    )
    ...
    return [
        Finding(
            ...
            "ERROR",
            "UX-09",
            "Destructive panel action lacks confirmation/risk gate",
        )
    ]
...
def detect_ux10(path, raw, src):
    ...
    for m in re.finditer(r'"(?:\\.|[^"\\\n])*"', raw):
        ...
        if not re.search(r"(?:error|failed|invalid)", literal, re.I):
            continue
        if re.search(
            r"retry|check|enable|disable|contact|reduce|increase|reconnect", literal, re.I
        ):
            continue
        line, col = _line_col(raw, m.start())
        return [
            Finding(
                path, line, col, "WARN", "UX-10", "Panel error text lacks an actionable remediation"
            )
        ]
    return []
...
def detect_ux11(path, raw, src):
    ...
    return [
        Finding(
            ...
            "WARN",
            "UX-11",
            "Decorative glyph/emoji in panel label can reduce terminal portability",
        )
    ]
...
def detect_ux12(path, raw, src):
    ...
    m = re.search(
        r"\bARGB\s*\([^)]*\)|(?:color|OBJPROP_(?:COLOR|BGCOLOR|BORDER_COLOR))[^;\n]{0,120}0x[0-9A-Fa-f]{8}",
        src,
        re.I,
    )
    if not m or re.search(r"ui_token|panel_(?:bg|text|status)|VCK_COLOR", src, re.I):
        return []
    line, col = _line_col(src, m.start())
    return [
        Finding(
            path, line, col, "WARN", "UX-12", "Raw ARGB color is not visibly bound to a UI token"
        )
    ]
...
def detect_ui_perf02(path, raw, src):
    for m in re.finditer(
        r"\b(?:RenderPanel|Render|DrawPanel)\s*\([^)]*\)\s*\{(?P<body>.*?)\n?\}", src, re.S
    ):
        bad = re.search(
            r"\b(?:OrderSend|Buy|Sell|PositionClose|WebRequest|FileOpen|iCustom|iMA|iRSI|iATR|PositionsTotal|AccountInfoDouble)\s*\(",
            m.group("body"),
        )
        if bad:
            line, col = _line_col(src, m.start("body") + bad.start())
            return [
                Finding(
                    path,
                    line,
                    ...
                    "ERROR",
                    "UI-PERF-02",
                    "Renderer calls execution/network/heavy data API instead of consuming a snapshot",
                )
            ]
    return []
...
def detect_ui_perf01(path, raw, src):
    ...
    """
    panel_call = re.compile(
        r"\b(?:RenderPanel|DrawPanel|UpdatePanel|ObjectSetString|ObjectSetInteger|ObjectCreate)\s*\("
    )
    for name, body, start in _bodies(src):
        if name != "OnTick":
            continue
        ...
        if not m:
            continue
        guarded = re.search(
            r"dirty|budget|throttle|last_(?:render|update|redraw)|GetTickCount|elapsed|max_render",
            body,
            re.I,
        )
        if guarded:
...
def detect_ui_perf03(path, raw, src):
    ...
    pairs = (
        ...
        (
            r"\bCCanvas\b|\bCreateBitmapLabel\s*\(",
            r"\b(?:Destroy|Delete)\s*\(",
            "canvas created without Destroy",
        ),
        (
            r"\biCustom\s*\(",
            r"\bIndicatorRelease\s*\(",
            "indicator handle without IndicatorRelease",
        ),
    )
...
BEST_UI_DETECTORS = [
    ...
    ("UX-11", detect_ux11),
    ("UX-12", detect_ux12),
    ("UI-PERF-01", detect_ui_perf01),
    ("UI-PERF-02", detect_ui_perf02),
    ("UI-PERF-03", detect_ui_perf03),
    ("UI-PERF-04", detect_ui_perf04),
]
