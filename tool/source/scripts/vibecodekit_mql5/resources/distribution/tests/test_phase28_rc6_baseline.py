...
def test_current_version_triple_is_consistent():
    ...
    assert f'version = "{current}"' in pyproject
    assert catalog["kit_version"] == current
    assert contract["kit"]["version"] == current
...
def test_rc5_artifacts_remain_historical_inputs():
    ...
    if source_zip.is_file() or wheel.is_file():
        ...
        # Standalone RC6-era source ZIP and wheel channels intentionally exclude
