...
from vibecodekit_mql5.ui_contract import default_ui_contract, validate  # noqa: E402
from vibecodekit_mql5 import retro_checker  # noqa: E402
...
def _out_dir() -> Path:
    ...
    ap.add_argument(
        "--out",
        default=None,
        help="Directory for UI-E2E-REPORT.html / UI-E2E-RESULT.json (default: ./evidence/ui).",
    )
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="vck-ui-e2e-") as td:
        ...
        contract["ui_contract"]["rows"] = [
            {
                "id": "drawdown",
                "label": "Drawdown",
                "source": "account_equity_peak",
                "refresh": "on_position_change",
                "stale_after_ms": 1000,
            }
        ]
        contract["ui_contract"]["controls"] = [
            {
                "id": "emergency_stop",
        ...
        results.append(("UI-CONTRACT validation", not errors, errors))
        ...
        evdir = project / "evidence/ui"
        evdir.mkdir(parents=True)
        now = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
        (evdir / "contract-conformance.json").write_text(
            json.dumps(
                {
                    "status": "PASS",
                    "source": "offline_contract_validator",
                    "recorded_at_utc": now,
                    "contract_sha256": hashlib.sha256(
                        (project / "UI-CONTRACT.json").read_bytes()
                    ).hexdigest(),
                }
            ),
            encoding="utf-8",
        )
        (evdir / "claims.json").write_text(
            json.dumps(
                {
                    "status": "PASS",
                    "source": "offline_contract_validator",
        ...
        (evdir / "performance-profile.json").write_text(
            json.dumps(
                {
                    "status": "PASS",
                    "source": "offline_static_profile",
                    "recorded_at_utc": now,
                    "render_us_p95": 410,
                    "ontick_extra_us_p95": 12,
                }
            ),
            encoding="utf-8",
        )
        spec = {
            "project": {"name": "PanelDemo"},
        ...
        static_ok = (
            "ChartRedraw" not in source.split("void OnTick", 1)[1].split("void OnTimer", 1)[0]
            and "QueueIntent" in source
        )
        results.append(
            (
                "Hot-path isolation static check",
                static_ok,
                {"ontick_has_chartredraw": not static_ok},
            )
        )
    test = subprocess.run(
        [sys.executable, "-m", "vibecodekit_mql5.selftest"],
        cwd=ROOT,
    ...
    results.append(
        (
            ...
            test.returncode == 0,
            {"returncode": test.returncode, "tail": test.stdout[-1200:] + test.stderr[-500:]},
        )
    )
    elapsed = round(time.perf_counter() - started, 3)
    payload = {
        "suite": "VibeCodeKit MQL5 v3.1 UI/Panel E2E demo",
    ...
    (out_dir / "UI-E2E-RESULT.json").write_text(
        json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8"
    )
    rows = "".join(
        f"<tr><td>{n}</td><td class='{'pass' if p else 'fail'}'>{'PASS' if p else 'FAIL'}</td><td><pre>{json.dumps(d, ensure_ascii=False, indent=2)}</pre></td></tr>"
