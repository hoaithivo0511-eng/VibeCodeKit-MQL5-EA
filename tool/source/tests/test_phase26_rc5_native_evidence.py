...
from vibecodekit_mql5 import rc5_native_gate as gate
...
def _write_project(tmp_path: Path, binding: dict[str, str]) -> Path:
    ...
    manifest = {
        ...
        "compile": {
            ...
            "tool_version": "MetaEditor build 5260",
            "errors": 0,
            "mq5_sha256": sha256_file(project / gate.SOURCE_MQ5),
        ...
        "backtest": {
            ...
            "native_scenarios": {
                "async_fill": {
                    ...
                    "sha256": sha256_file(project / gate.ASYNC_REPORT),
                    "status": "PASS",
                },
                "restart_recovery": {
                    "path": gate.RESTART_REPORT.as_posix(),
                    "sha256": sha256_file(project / gate.RESTART_REPORT),
                    "status": "PASS",
                },
            },
        },
...
def test_async_duplicate_order_is_release_failure(tmp_path: Path, monkeypatch):
    ...
    data = json.loads(async_path.read_text(encoding="utf-8"))
    data["duplicate_order_count"] = 1
    ...
    result = gate.validate_rc5_native_evidence(repo, project)
    assert result.status == "FAIL"
    assert "async-fill evidence reports duplicate orders" in result.errors
...
def test_restart_requires_terminal_proof_or_operator_required(tmp_path: Path, monkeypatch):
    ...
    data = json.loads(restart_path.read_text(encoding="utf-8"))
    data["resolution"] = "BLIND_RETRY"
...
def test_compile_log_error_count_cannot_be_overridden_by_manifest(tmp_path: Path, monkeypatch):
    ...
    result = gate.validate_rc5_native_evidence(repo, project)
    assert result.status == "FAIL"
    assert "MetaEditor compile reports 2 error(s)" in result.errors
...
def test_generic_provenance_fail_remains_fail(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda _: _prov("FAIL", errors=["runner key is not pinned"]),
    )
    result = gate.validate_rc5_native_evidence(repo, project)
    assert result.status == "FAIL"
    assert "runner key is not pinned" in result.errors
...
def test_generic_provenance_incomplete_remains_blocked(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        gate,
        "validate_release_provenance",
        lambda _: _prov("INCOMPLETE", missing=["VCK_RUNNER_PUBLIC_KEY_B64"]),
    )
    result = gate.validate_rc5_native_evidence(repo, project)
    assert result.status == "BLOCKED"
    assert "VCK_RUNNER_PUBLIC_KEY_B64" in result.missing
