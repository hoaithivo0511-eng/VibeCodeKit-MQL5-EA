...
//+------------------------------------------------------------------+
//| NorthTrend.mq5 | EA-IR 43b0cf5c9748c8637d5004b489401ed752ff864579204a3b7899b4bdb5f79dff
//+------------------------------------------------------------------+
#property strict
#property version "3.30"
#include <NorthTrend/Config.mqh>
#include <NorthTrend/Core/AsyncTradeExecutor.mqh>
#include <NorthTrend/Core/PositionBook.mqh>
#include <NorthTrend/Core/TradeEventReducer.mqh>
#include <NorthTrend/Signal/EntryEngine.mqh>
#include <NorthTrend/Risk/GridRiskGuard.mqh>
#include <NorthTrend/Exit/BasketCloseEngine.mqh>
#include <NorthTrend/State/PersistentStateStore.mqh>
#include <NorthTrend/Telemetry/StructuredLogger.mqh>
#include <NorthTrend/Telemetry/MfeMaeLogger.mqh>
...
int OnInit(){g_symbol=StringLen(InpTradeSymbol)>0?InpTradeSymbol:_Symbol;if(!SymbolSelect(g_symbol,true))return INIT_FAILED;if((VCK_USE_DCA||VCK_USE_HEDGE||VCK_USE_HEDGE_ZONE||VCK_USE_REVERSE_ENTRY||VCK_USE_LOT_BALANCE)&&(ENUM_ACCOUNT_MARGIN_MODE)AccountInfoInteger(ACCOUNT_MARGIN_MODE)!=ACCOUNT_MARGIN_MODE_RETAIL_HEDGING){Print("Composition requires MT5 hedging account");return INIT_FAILED;}g_pip=PipSize();if(g_pip<=0||!Entry.Init(g_symbol,InpSignalTimeframe))return INIT_FAILED;MathSrand((int)GetTickCount());Trade.Configure(InpMagic,g_symbol,InpSignalTimeframe,InpMaxSpreadPips,InpAsyncExecution);EventReducer.Configure(InpMagic,g_symbol);StateStore.Configure(InpMagic,g_symbol);StateStore.Load(g_ea_enabled,g_new_cycle,g_stop_buy,g_stop_sell,g_lottery_factor);StateStore.LoadExtended(g_daily_halt_day,g_balance_day,g_day_start_balance,g_persisted_peak,g_hedge_zone,g_zone_phase,g_zone_cycle_id,g_zone_anchor_position_id,g_zone_low,g_zone_high,g_cooldown_until);GridRisk.Init(g_persisted_peak);Log.Configure("NorthTrend");MfeMae.Configure("NorthTrend");Trade.Reconcile();VCKSideStats buy,sell;Book.Collect(g_symbol,InpMagic,POSITION_TYPE_BUY,buy);Book.Collect(g_symbol,InpMagic,POSITION_TYPE_SELL,sell);ReconcileHedgeZoneState(buy,sell);CreatePanel();Log.Event("INIT","EA initialized");return INIT_SUCCEEDED;}
