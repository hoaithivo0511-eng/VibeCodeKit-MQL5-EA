#!/usr/bin/env python3
"""Build and verify the RC5 candidate distribution from tracked tool/source.

Task 09 is a package-integration gate only. It deliberately keeps
release_eligible=false until Task 10 supplies trusted native evidence.
"""
...
ARTIFACT_SUMS = RELEASE_DIR / "RC5-ARTIFACTS.sha256"
COMPLETION_REPORT = RELEASE_DIR / "PR-09-COMPLETION.md"
...
def build_wheel() -> None:
    with tempfile.TemporaryDirectory(prefix="rc5-wheel-") as temp:
        subprocess.check_call(
            [sys.executable, "-m", "build", "--wheel", "--outdir", temp, str(SOURCE)],
            cwd=ROOT,
        )
...
def build_candidate_manifest(build_input_sha: str, source_manifest: dict) -> dict:
    ...
    manifest = {
        ...
        "release_blockers": [
            "Task 10 trusted native MetaEditor/MT5 compile evidence is pending",
            "Task 10 Strategy Tester evidence is pending",
            "Task 10 crash/restart recovery evidence is pending",
        ],
        "build_input_commit": build_input_sha,
...
def verify_candidate_manifest() -> None:
    ...
    if data.get("release_eligible") is not False:
        raise SystemExit("Task 09 candidate must remain release_eligible=false")
...
def verify_artifact_sums() -> None:
    ...
    if declared != expected:
        raise SystemExit("RC5 artifact SHA-256 manifest mismatch")
...
def cmd_build(args: argparse.Namespace) -> None:
    ...
    write_artifact_sums()
    print(f"RC5 candidate built from source tree {source_manifest['source_tree_sha']}")
...
def cmd_verify(_: argparse.Namespace) -> None:
    ...
    print("RC5 candidate package verification: PASS")
